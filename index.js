...
class TrieNode {
	...
	getWord() {
		// iterates through the parents to get the word. time complexity: O(k), k = word length
		const output = [];
		let node = this;
		while (node !== null) {
...
class Trie {
	...
	insert(word) {
		// inserts a word into the trie. time complexity: O(k), k = word length
	...
	contains(word) {
		// check if it contains a whole word. time complexity: O(k), k = word length
		let node = this.root;
		for (var i = 0; i < word.length; i++) {
			// for every character in the word
			if (node.children[word[i]]) {
...
function calcMostEfficientLayout(list, bruteforceTimeout) {
	...
	if (leftoverBytes <= 32)
		return { timedout: false, bestSlotCount: currentSlotCount + 1 };
	...
	for (const perm of genPermutations(list)) {
		if (Date.now() > timeoutAt) {
			return {
				timedout: true,
				bestSlotCount: currentSlotCount + bestSlotCount,
			}; // indicate timeout was triggered
		}
		...
		if (!slotCounTooHigh && slotCount < bestSlotCount) {
			...
			if (bestSlotCount === bestPossibleSlotCount) {
...
function getEnumByteSize(memberCount) {
	...
	throw new Error(
		`a struct cannot have this high number (${memberCount}) of members, it will exceed the block gas limit`
	);
}

function exec(args) {
	if (!args.bruteforce_timeout)
		args.bruteforce_timeout = DEFAULT_BRUTE_FORCE_TIMEOUT;
	...
	args.solidity_file_paths.forEach((solidity_file_path) => {
		...
		const structs = flattenDeep(
			ast.children
				.filter((child) => child.type === "ContractDefinition")
				.map((child) =>
					child.subNodes
						.filter((n) => n.type === "StructDefinition")
						.map((n) => ({ contract: child.name, ...n }))
				)
		);

		const enums = flattenDeep(
			ast.children
				.filter((child) => child.type === "ContractDefinition")
				.map((child) =>
					child.subNodes
						.filter((n) => n.type === "EnumDefinition")
						.map((n) => ({ contract: child.name, ...n }))
				)
		);
			...
			const structMembers = targetStruct.members.map((targetMember) => {
				...
				switch (targetMember.typeName.type) {
					case "UserDefinedTypeName":
						if (enums.length) {
							const foundEnum = enums.find(
								(_enum) => _enum.name === targetMember.typeName.namePath
							);
							if (foundEnum) {
								varKind = "enum";
					...
					case "ElementaryTypeName": {
						...
						switch (targetMemberType) {
							...
							case "address":
								...
								break;
							default:
								if (targetMemberType === "int" || targetMemberType === "uint")
									targetMemberType += "256";
								if (targetMemberType.startsWith("int")) {
									// intX
									varByteSize =
										parseInt(targetMemberType.match(intRegex)[1], 10) / 8;
									break;
								}
								if (targetMemberType.startsWith("uint")) {
									// uintX
									varByteSize =
										parseInt(targetMemberType.match(uintRegex)[1], 10) / 8;
									break;
								}
								if (targetMemberType.startsWith("bytes")) {
									// bytesX
									varByteSize = parseInt(
										targetMemberType.match(bytesRegex)[1],
										10
									);
									break;
								}
		...
		Object.keys(parsedStructs).forEach((contractName) => {
			Object.keys(parsedStructs[contractName]).forEach((structName) => {
				textOutput(
					`struct ${structName} { // file: ${uniqueFileName} | contract: ${contractName}\n`
				);
				...
				parsedStructs[contractName][structName].forEach((targetMember) => {
					...
					if (newMemberSize + bytesOccupiedInSlot > 32) {
						textOutput(
							"  //" +
								"-".repeat(10) +
								` end of slot | bytes taken: ${bytesOccupiedInSlot} | bytes free: ${
									32 - bytesOccupiedInSlot
								}\n`
						);
					...
					textOutput(
						`  ${input
							.split("\n")
							[targetMember.varLine].replace(/\/\/.+/, "")
							.trim()} // bytes: ${newMemberSize}`
					);
				});

				const originalSlotsTaken = slotsFilledCount;

				const totalBytesTaken =
					(slotsFilledCount - 1) * 8 + bytesOccupiedInSlot;

				// will try and get the most efficient layout, or timeout after x seconds and report best found until then
				const calcResult = calcMostEfficientLayout(
					parsedStructs[contractName][structName].map((x) => x.varByteSize),
					args.bruteforce_timeout
				);
				...
				inefficientStructs[uniqueFileName] = {
					...
					[contractName]: {
						...(inefficientStructs[uniqueFileName]
							? inefficientStructs[uniqueFileName][contractName]
							: {}),
						[structName]: {
							current: slotsFilledCount,
							optimized: optimizedSlotsTaken,
							timedout: calcResult.timedout,
						},
					},
				};
				...
				if (calcResult.timedout) {
					...
					textOutput(
						`  // best found result before timeout: slots that can be saved = ${
							slotsFilledCount - optimizedSlotsTaken
						}\n`
					);
				}
				textOutput(
					`\n} // slots that can be saved = ${
						slotsFilledCount - optimizedSlotsTaken
					}\n`
				);
			});
	...
	Object.keys(inefficientStructs).forEach((inputFilePath) => {
		Object.keys(inefficientStructs[inputFilePath]).forEach((contractName) => {
			Object.keys(inefficientStructs[inputFilePath][contractName]).forEach(
				(structName) => {
					const structSummary =
						inefficientStructs[inputFilePath][contractName][structName];
					if (structSummary.current > structSummary.optimized) {
						...
						if (bruteforceTimedOut) {
							textOutput(
								`// timed out: yes, after ${args.bruteforce_timeout}s`
							);
							textOutput(
								`// best found slots saved: ${
									structSummary.current - structSummary.optimized
								}`
							);
						} else {
							textOutput(
